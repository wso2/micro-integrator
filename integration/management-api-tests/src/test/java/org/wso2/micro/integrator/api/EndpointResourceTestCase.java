...
public class EndpointResourceTestCase extends ESBIntegrationTest {
    ...
    public void retrieveEndpoints() throws IOException {
        ...
        Assert.assertTrue(jsonResponse.get("list").toString().contains("AbcEndPoint"), "Assert failed " +
                "since expected resource name not found in the list");
        Assert.assertTrue(jsonResponse.get("list").toString().contains("HelloEndPoint"), "Assert failed " +
                "since expected resource name not found in the list");
    }
    ...
    public void retrieveSearchedEndpoints() throws IOException {
        ...
        String responsePayload = sendHttpRequestAndGetPayload("HelloEndPoint");
        JSONObject jsonResponse = new JSONObject(responsePayload);
    ...
    private String sendHttpRequestAndGetPayload(String searchKey) throws IOException {
        ...
        String endpoint = "https://" + hostName + ":" + (DEFAULT_INTERNAL_API_HTTPS_PORT + portOffset) + "/management/endpoints";
        if (searchKey != null) {
