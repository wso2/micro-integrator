...
public class InboundEndpointResourceTestCase extends ESBIntegrationTest {
    ...
    public void retrieveInboundEPs() throws IOException {
        ...
        Assert.assertTrue(jsonResponse.get("list").toString().contains("HL7_Inbound"), "Assert failed " +
    ...
    public void retrieveSearchedInboundEPs() throws IOException {
        ...
        String responsePayload = sendHttpRequestAndGetPayload("HL7_");
        JSONObject jsonResponse = new JSONObject(responsePayload);
        Assert.assertEquals(jsonResponse.get("count"), 1, "Assert Failed due to the mismatch of " +
                "actual vs expected resource count");
    }
    ...
    private String sendHttpRequestAndGetPayload(String searchKey) throws IOException {
        ...
        String endpoint = "https://" + hostName + ":" + (DEFAULT_INTERNAL_API_HTTPS_PORT + portOffset) + "/management/inbound-endpoints";
        if (searchKey != null) {
