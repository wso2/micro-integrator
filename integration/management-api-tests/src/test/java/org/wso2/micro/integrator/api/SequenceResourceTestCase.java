...
public class SequenceResourceTestCase extends ESBIntegrationTest {
    ...
    public void retrieveSequences() throws IOException {
        ...
        Assert.assertEquals(jsonResponse.get("count"), 5);
        Assert.assertTrue(jsonResponse.get("list").toString().contains("AbcSequence"));
        Assert.assertTrue(jsonResponse.get("list").toString().contains("fault"));
        Assert.assertTrue(jsonResponse.get("list").toString().contains("main"));
    }
