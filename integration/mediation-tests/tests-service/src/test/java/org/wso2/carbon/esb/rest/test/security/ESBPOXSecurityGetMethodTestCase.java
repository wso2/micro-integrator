...
public class ESBPOXSecurityGetMethodTestCase extends ESBIntegrationTest {
    ...
    @Test(groups = {
            "wso2.esb" }, description = "test pox security with super tenant credentials") public void testGetQuote()
        ...
        HttpsResponse response = HttpsURLConnectionClient
                .getWithBasicAuth(securedRestURL, "symbol=IBM", userInfo.getUserName(), userInfo.getPassword());
        assertTrue(response.getData().contains(":name>IBM Company</"), "getQuote doesn't return expected values");
        assertTrue(response.getData().contains(":symbol>IBM</"), "getQuote doesn't return expected values");
    }
    ...
    @Test(groups = {
            "wso2.esb" }, description = "test pox security with invalid user credentials", dependsOnMethods = "testGetQuoteByUser") public void testGetQuoteWithInvalidCredentials()
            throws Exception {
        String securedRestURL = getProxyServiceURLHttps(SERVICE_NAME) + "/getSimpleQuote";
        boolean status = false;
        HttpsResponse response = null;
        try {
            response = HttpsURLConnectionClient
                    .getWithBasicAuth(securedRestURL, "symbol=IBM", "invalidUserName", "InvalidPassword");
    ...
    @Test(groups = {
            "wso2.esb" }, description = "test pox security with invalid user group", dependsOnMethods = "testGetQuoteWithInvalidCredentials") public void testGetQuoteWithInvalidGroup()
            throws Exception {
        String securedRestURL = getProxyServiceURLHttps(SERVICE_NAME) + "/getSimpleQuote";
        ...
        boolean status = false;
        try {
            response = HttpsURLConnectionClient.getWithBasicAuth(securedRestURL, "symbol=IBM",
