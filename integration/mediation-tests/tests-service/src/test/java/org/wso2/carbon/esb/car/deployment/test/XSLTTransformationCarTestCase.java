...
public class XSLTTransformationCarTestCase extends ESBIntegrationTest {
    ...
    @BeforeClass(alwaysRun = true) protected void uploadCarFileTest() throws Exception {
        ...
        carbonAppUploaderClient = new CarbonAppUploaderClient(context.getContextUrls().getBackEndUrl(),
                getSessionCookie());
        carbonAppUploaderClient.uploadCarbonAppArtifact("xslt-transformation-car_1.0.0.car", new DataHandler(
                new FileDataSource(new File(getESBResourceLocation() + File.separator + "car" + File.separator
                        + "xslt-transformation-car_1.0.0.car"))));
    ...
    @Test(groups = {
        ...
        Assert.assertTrue(esbUtils.isProxyDeployed(context.getContextUrls().getBackEndUrl(), getSessionCookie(),
                "xsltTransformationProxy"), "Pass Through Proxy service deployment failed");
        Assert.assertTrue(isResourceExist("/_system/config/transform.xslt"), "transform.xslt not found on registry");
        Assert.assertTrue(isResourceExist("/_system/config/transform_back.xslt"),
                "transform.xslt not found on registry");

        OMElement response = null;
        try {
            response = axis2Client.sendCustomQuoteRequest(getProxyServiceURLHttp("xsltTransformationProxy"), null,
                    "XSLTTransformation");
        ...
        Assert.assertTrue(response.toString().contains("XSLTTransformation"),
    ...
    @Test(groups = { "wso2.esb" }, description = "test proxy service invocation", dependsOnMethods = {
        ...
        Assert.assertTrue(esbUtils.isEndpointUnDeployed(context.getContextUrls().getBackEndUrl(), getSessionCookie(),
                "stockQuoteServiceEndpoint"), "stockQuoteServiceEndpoint Endpoint deployment failed");
        Assert.assertTrue(esbUtils.isProxyUnDeployed(context.getContextUrls().getBackEndUrl(), getSessionCookie(),
                "xsltTransformationProxy"), "xsltTransformationProxy Proxy service deployment failed");

        Assert.assertFalse(isResourceExist("/_system/config/transform.xslt"),
                "transform.xslt deployed by car file not removed from registry");
        Assert.assertFalse(isResourceExist("/_system/config/transform_back.xslt"),
                "transform_back.xslt deployed by car file not removed from registry");
