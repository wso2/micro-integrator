...
public class MalformedHTTPRequestTest extends ESBIntegrationTest {
    ...
    public void testMalformedURL() throws Exception {
        ...
        for (int i = 0; i < 3; i++) {
            try {
                ...
                httpClientUtil.sendGetRequest(
                        getProxyServiceURLHttp("simpleProxy").replace("services", " services") + "?WSO2", null);
            } catch (Exception e) {
            }
        }
        //check whether ESB is still stable by sending a correct request
        OMElement response = axis2Client.sendSimpleQuoteRequest(getProxyServiceURLHttp("simpleProxy"), null, "WSO2");
        ...
        //check whether ESB is still stable by sending a correct request
        OMElement response2 = axis2Client.sendSimpleQuoteRequest(getProxyServiceURLHttp("simpleProxy"), null, "WSO2");
        ...
        //check whether ESB is still stable by sending a correct request
        OMElement response3 = axis2Client.sendSimpleQuoteRequest(getProxyServiceURLHttp("simpleProxy"), null, "WSO2");
