 ...
 */
public class SMB2FileTransferTestCaseCheckContent extends ESBIntegrationTest {

    private static final Log LOGGER = LogFactory.getLog(SMB2FileTransferTestCaseCheckContent.class);

    private File inputFolder;
    private File outputFolder;
    private File originalFolder;
    private HashMap<String, String> inputFiles = new HashMap<>();
    ...
    public void fileTransferTestCheckForContent() throws XMLStreamException, IOException, InterruptedException {
        ...
        Awaitility.await().atMost(120, TimeUnit.SECONDS).until(checkForOutputFileContent());
    }
     ...
     * @throws IOException
     */
    public void copyDirectory(File sourceDir, File destDir) throws IOException, NoSuchAlgorithmException {
        FileUtils.copyDirectory(sourceDir, destDir);
        File[] files = inputFolder.listFiles();
        for (File file : files) {
            inputFiles.put(file.getName(), Utils.checksum(file));
        }
    }

    /*
     * Check whether all the file content have been copied from in to out
     * */
    private Callable<Boolean> checkForOutputFileContent() {
        return new Callable<Boolean>() {
            @Override
            public Boolean call() throws IOException, NoSuchAlgorithmException {
                File[] files = outputFolder.listFiles();
                if (files.length < 100) {
                    return false;
                }
                for (File file : files) {
                    String matchingInPutFileMd5Sum = inputFiles.get(file.getName());
                    String outPutFileMd5Sum = Utils.checksum(file);
                    if (!outPutFileMd5Sum.equals(matchingInPutFileMd5Sum)) {
                        return false;
                    }
                }
                return true;
