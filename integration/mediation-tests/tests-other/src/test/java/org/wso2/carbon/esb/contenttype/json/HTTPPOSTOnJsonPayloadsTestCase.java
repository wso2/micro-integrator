...
public class HTTPPOSTOnJsonPayloadsTestCase extends ESBIntegrationTest {
    ...
    @Test(groups = {
            "wso2.esb" }, description = "Tests POST method with application/json content type") public void testHTTPPostRequestJSONScenario()
            throws Exception {

        String JSON_PAYLOAD = "{\"album\":\"Hotel California\",\"singer\":\"Eagles\"}";

        WebResource webResource = client.resource(getProxyServiceURLHttp("JsonHTTPPostProxy"));

        // sending post request
        ClientResponse postResponse = webResource.type("application/json").post(ClientResponse.class, JSON_PAYLOAD);
