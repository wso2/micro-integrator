...
public class SmooksMediatorXMLtoXMLTransformTestCase extends ESBIntegrationTest {
    ...
    private final String COMMON_FILE_LOCATION = getClass().getResource(
    ...
    private final String[] ORDER_ITEMS = { "Pen", "Book", "Bottle", "Note Book", "Pencil", "Chocolate", "Bun", "Banana",
            "Hat", "Toffee" };
    ...
    public void testXMLtoXMLTransformationUsingSmooksMeidator() throws Exception {
        ...
        File bfile = new File(COMMON_FILE_LOCATION + "test" + File.separator + "in" + File.separator
                + "synapse_sample_658_input.xml");
        ...
        for (int i = 0; i < 10; i++) {
            fileContents = FileUtils.readFileToString(
                    new File(TARGET_FILE_LOCATION + File.separator + "order-" + ORDER_ID + "-" + (i + 1) + ".xml"));
    ...
    private void addVFSProxy() throws Exception {
        ...
        addProxyService(AXIOMUtil.stringToOM(
                "<proxy xmlns=\"http://ws.apache.org/ns/synapse\" name=\"StockQuoteProxy\" transports=\"vfs\">\n"
                        ...
                        + COMMON_FILE_LOCATION + "test" + File.separator + "in" + File.separator + "</parameter>\n"
                        + "        <parameter name=\"transport.vfs.ContentType\">application/xml</parameter>\n"
                        + "        <parameter name=\"transport.vfs.FileNamePattern\">.*\\.xml</parameter>\n"
                        + "        <parameter name=\"transport.PollInterval\">5</parameter>\n"
                        + "        <!--CHANGE-->\n"
                        + "        <parameter name=\"transport.vfs.MoveAfterProcess\">file://" + COMMON_FILE_LOCATION
                        + "test" + File.separator + "out" + File.separator + "</parameter>\n"
                        + "        <!--CHANGE-->\n"
                        + "        <parameter name=\"transport.vfs.MoveAfterFailure\">file://" + COMMON_FILE_LOCATION
                        + "test" + File.separator + "out" + File.separator + "</parameter>\n"
    ...
    private void uploadResourcesToConfigRegistry() throws Exception {
        ...
        resourceAdminServiceStub.addCollection("/_system/config/", "smooks", "", "Contains smooks config files");
        resourceAdminServiceStub
                .addResource("/_system/config/smooks/synapse_config_658.xml", "application/xml", "xml files",
