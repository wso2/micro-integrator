...
public class EnrichIntegrationReplaceEnvelopTestCase extends ESBIntegrationTest {
    ...
    public void testReplaceEnvelop() throws AxisFault {
        ...
        assertEquals(response.getFirstElement().getLocalName().toString(), "request", "Local name does not match");
        assertEquals(response.getFirstElement().getFirstChildWithName(new QName("http://services.samples", "symbol"))
