...
public class PropertyIntegrationDisableAddressingForOutMessagesTestCase extends ESBIntegrationTest {
    ...
    public void testDisableAddressingForOutMessages() throws Exception {
        ...
        assertEquals(response.getFirstElement().getFirstElement().getQName().getPrefix(), "wsa", "Property not set");
        assertEquals(response.getFirstElement().getFirstElement().getQName().getNamespaceURI(),
                "http://www.w3.org/2005/08/addressing", "Property not set");
        assertTrue(response.getFirstElement().toString().contains("wsa:MessageID"), "Property not set");
    }
