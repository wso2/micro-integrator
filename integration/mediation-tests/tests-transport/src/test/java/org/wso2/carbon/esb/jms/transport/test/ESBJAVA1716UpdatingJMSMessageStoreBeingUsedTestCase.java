...
public class ESBJAVA1716UpdatingJMSMessageStoreBeingUsedTestCase extends ESBIntegrationTest {
    ...
    public void updateMessageStoreBeingUsedTest() throws Exception {
        ...
        for (int i = 0; i < (afterLogSize - beforeLogSize); i++) {
            Assert.assertFalse(logs[i].getMessage()
                            .contains("synapse.message.processor.quartz.JMSTestMessageProcessor-forward job threw an"),
                    "Exception observed in backend when editing message store which is used by message processor > "
                            + logs[i].getMessage());
        }
