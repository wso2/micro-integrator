...
public class MailToTransportActionAfterProcessMoveTestCase extends ESBIntegrationTest {
    ...
    public void initialize() throws Exception {
        ...
        OMElement mailToProxyOMElement = AXIOMUtil.stringToOM(FileUtils.readFileToString(new File(
                getESBResourceLocation() + File.separator + "mailTransport" + File.separator +
                        "mailTransportReceiver" + File.separator + "mail_transport_move.xml")));
        Utils.deploySynapseConfiguration(mailToProxyOMElement,
                "MailTransportMove","proxy-services",
                true);
    ...
    @Test(groups = { "wso2.esb" }, description = "Test email transport action after process move", enabled = false)
